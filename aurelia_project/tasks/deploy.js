...
let buckets = CLIOptions.hasFlag('buckets') ? CLIOptions.getFlagValue("buckets").split(",") : ["admin.cpafrance.fr"];
...
let functions = buckets.map(bucket => function (_done) {
    ...
    gulp.src(toUpload, { base: './' })
        .pipe(uploader({ Bucket: bucket, ACL: 'public-read', CacheControl: 'no-cache' }, uploaderOptions))
        .on('end', () => {
